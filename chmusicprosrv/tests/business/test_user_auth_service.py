...
class TestUserAuthService:
    ...
    def test_generate_jwt_token_contains_correct_payload(self, auth_service):
        ...
        assert "role" not in payload  # Legacy role removed
        assert "iat" in payload  # Issued at
        assert "exp" in payload  # Expiration

    def test_generate_jwt_token_contains_domain_claims(self, auth_service):
        """Test that JWT token contains domain claims when specified"""
        user_id = "123e4567-e89b-12d3-a456-426614174000"
        email = "test@example.com"

        token = auth_service.generate_jwt_token(user_id, email, active_domain_id="domain-123", domain_role="admin")

        payload = jwt.decode(token, JWT_SECRET_KEY, algorithms=[JWT_ALGORITHM])
        assert payload["active_domain_id"] == "domain-123"
        assert payload["domain_role"] == "admin"
