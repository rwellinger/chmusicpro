...
def upgrade() -> None:
    ...
    op.execute("DROP INDEX IF EXISTS ix_api_costs_monthly_provider_year_month")
    op.execute("""
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_provider_org_year_month') THEN
                ALTER TABLE api_costs_monthly DROP CONSTRAINT uq_provider_org_year_month;
            END IF;
        END $$;
    """)
    ...
    op.execute("""
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_project_image') THEN
                ALTER TABLE project_image_references DROP CONSTRAINT uq_project_image;
            END IF;
    ...
    op.execute("""
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_release_project') THEN
                ALTER TABLE release_project_references DROP CONSTRAINT uq_release_project;
            END IF;
        END $$;
    """)
    op.alter_column(
