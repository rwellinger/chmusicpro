-- ============================================================
-- Mac Prompt Templates - Complete Seed Script
-- ============================================================
-- Self-contained script that seeds ALL prompt templates with
-- Mac-optimized Ollama models. Uses UPSERT (ON CONFLICT) to
-- create or overwrite existing entries.
--
-- Available Mac models:
--   qwen3:8b     (~6 GB)  - fast, titles & simple tasks
--   qwen3:30b    (~20 GB) - MoE (30B/3B active), quality tasks
--
-- Hardware: Apple Silicon Mac with 32GB+ unified memory
--
-- Usage (local):
--   cat scripts/db/seed_mac_prompt_templates.sql | docker exec -i postgres psql -U chmusicpro -d chmusicpro
--
-- ============================================================
...
-- Helper function for upsert operations (includes provider column)
...
BEGIN
    INSERT INTO prompt_templates (
        ...
        version, provider, model, temperature, max_tokens, active,
        created_at, updated_at
    ) VALUES (
        p_category, p_action, p_pre_condition, p_post_condition, p_description,
        p_version, p_provider, p_model, p_temperature, p_max_tokens, p_active,
        NOW(), NOW()
    )
...
-- ============================================================
-- DESCRIPTION TEMPLATES (qwen3:30b)
-- ============================================================
...
Output: Ein atmosphaerischer Song ueber die Schoenheit der Nacht und die Kraft der Stille. Mit poetischen Bildern von Mondlicht und Schatten entfaltet sich eine Geschichte der inneren Einkehr und Selbstfindung. Die melancholische Stimmung wird durch hoffnungsvolle Momente durchbrochen.
...
Input (German): "Ein atmosphaerischer Song ueber die Schoenheit der Nacht und die Kraft der Stille. Mit poetischen Bildern von Mondlicht und Schatten..."
Output: Poetische Reise durch die Nacht - melancholisch, hoffnungsvoll, atmosphaerisch.
...
Input (German): "Ein atmosphaerischer Song ueber die Schoenheit der Nacht und die Kraft der Stille..."
Output: Nacht, Atmosphaerisch, Melancholisch, Poetisch, Stille, Mondlicht, Introspektiv, Hoffnungsvoll, Dunkel, Emotional
...
-- ============================================================
-- IMAGE TEMPLATES (qwen3:30b + qwen3:8b for fast)
-- ============================================================
...
Input (German lyrics): "Der Duft von frisch geschnittenem Gras vermischt sich mit dem kuehlen Seufzer des Regens. Adam beobachtet, wie ein heller Fruehling wie eine aufsteigende Flut emporsteigt..."
...
Examples:
Input (German): "Ein gemuetlicher Winterabend mit Schnee"
Output: "A cozy winter evening with snow"

Input (German): "Sonnenuntergang am Meer, traumhaft schoen"
...
SELECT upsert_prompt_template(
    ...
    'Fast title generation optimized for speed with qwen3:8b (auto-generation fallback)',
    '1.1',
    'ollama',
    'qwen3:8b',
    0.5,
...
-- ============================================================
-- WORKSHOP TEMPLATES (qwen3:30b + qwen3:8b)
-- ============================================================
...
-- ============================================================
-- PROMPT ENGINEERING TEMPLATES (qwen3:8b - fast)
-- ============================================================
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '============================================================';
    RAISE NOTICE 'Mac Seeding completed successfully!';
    RAISE NOTICE '============================================================';
...
-- Show templates grouped by model
SELECT
    model,
    COUNT(*) as count,
    string_agg(category || '/' || action, ', ' ORDER BY category, action) as templates
